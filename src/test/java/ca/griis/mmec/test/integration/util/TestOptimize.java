...
public class TestOptimize extends OntopTester {
    ...
    public void runTest() throws Exception {
        try (Connection connection = LocalJDBCConnectionUtils.createConnection((OntopSQLCredentialSettings) configuration.getSettings())) {
            JDBCMetadataProviderFactory metadataProviderFactory = configuration.getInjector().getInstance(JDBCMetadataProviderFactory.class);
            ...
            for (String sourceQuery : queries) {
                ...
                AtomPredicate rdfAnswerPredicate = atomFactory.getRDFAnswerPredicate(variablesProjection.size());

                DistinctVariableOnlyDataAtom projectionAtom = atomFactory.getDistinctVariableOnlyDataAtom(rdfAnswerPredicate, variablesProjection.toArray(new Variable[0]));
                ConstructionNode constructionNode = iqFactory.createConstructionNode(projectionAtom.getVariables(), substitutionFactory.getSubstitution(variablesProjection, variablesConstruction));

                IQ iq = iqFactory.createIQ(projectionAtom, iqFactory.createUnaryIQTree(constructionNode, tree));
