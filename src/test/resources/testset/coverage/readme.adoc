...
2. Importer le fichier OntoGraph.dot dans cas_limites_individuation.graffle en glissant et déposant l'icone du fichier sur l'icone de OmniGraffle dans la barre de tâche.
3. S'assurer que 1000-BW_OntoRelCat_ins est bien conforme à la version de l'OntoRelCat utilisé dans le projet
4. Créer une instance de la base de données
5. Générer la figure schemaSpy à l'aide de la commande: `schemaspy-griis-application -t pgsql11 -dp $PG_DRIVER -vizjs -imageformat svg -schemas 'BW' -host localhost -port 25432 -db test -u test -p test -o ./src/test/resources/testset/coverage/humanBodyWeightOntology/SchemaSpy -desc "Documentation de l'OntoRel BW."`
6. Importer le fichier "src/test/resources/testset/coverage/SchemaSpy/BW/diagrams/summary/relationships.real.large.svg" dans le fichier "cas_limites_individuation.graffle" en glissant et déposant l'icone du fichier dans un nouveau calque.
