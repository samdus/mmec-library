...
public class DataPropertyProjectionTransformer extends DefaultRecursiveIQTreeVisitingTransformer {
    ...
    @Inject
    protected DataPropertyProjectionTransformer(IntermediateQueryFactory iqFactory, TermFactory termFactory,
...
//                EXTENSIONAL "TABLE2"(0:m1m0,2:o1m2)
    ...
    private ImmutableTerm getTermOrChangeRDFFunctionForConversionFunction(IQTree tree, Map.Entry<Variable, ImmutableTerm> substitutionEntry) {
        if (substitutionEntry.getValue() instanceof NonGroundFunctionalTerm term
            ...
            try {
                //TODO: Fetch OntoRelId from the configuration
                DBTermType sqlDataType = ontoRelCatRepository.getSQLType("OntoRelCat_simple", rdfDatatype.getIRI().getIRIString());
