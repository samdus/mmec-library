...
public class MmecOntopMappingOntologyConfigurationImpl extends OntopMappingConfigurationImpl
  ...
  protected MmecOntopMappingOntologyConfigurationImpl(OntopMappingSettings settings,
      OntopMappingOntologyOptions options) {
  ...
  protected MmecOntopMappingOntologyConfigurationImpl(OntopMappingSettings settings,
      OntopMappingOntologyOptions options, Supplier<Injector> injectorSupplier) {
  ...
  private Optional<OWLOntology> loadOntologyFromFile() throws OWLOntologyCreationException {
    ...
    if (options.ontologyFile.isPresent()) {
      owlOntology =
          Optional.of(manager.loadOntologyFromOntologyDocument(options.ontologyFile.get()));
  ...
  private Optional<ImmutableSet<RDFFact>> loadFactsFromFile() throws FactsException {
    ...
    try {
      if (options.factsFile.isPresent()) {
        this.factsFile =
            Optional.of(loadFactsWithReader(new FileReader(options.factsFile.get()), format.get()));
        ...
        this.factsFile = Optional.of(loadFactsWithReader(
            new InputStreamReader(options.factsURL.get().openStream()), format.get()));
      }
    } catch (IOException e) {
      throw new FactsException(e);
    } catch (RDFParseException | RDFHandlerException | IllegalArgumentException e) {
      throw new FactsException("An error occured while parsing the facts file: " + e.getMessage(),
          e);
    }
   ...
   */
  private ImmutableSet<RDFFact> loadFactsWithReader(Reader reader, RDFFormat format)
      throws FactsException {
    RDFParser parser = Rio.createParser(format);
    Model model = new LinkedHashModel();
    parser.setRDFHandler(new StatementCollector(model));
    try {
      parser.parse(reader, options.factsBaseIRI
          .orElseGet(() -> String.format("http://%s.example.org/data/", UUID.randomUUID())));
    } catch (IOException e) {
      throw new FactsException(e);
    }
  ...
  /**
   * Converts RDF4J triples/quadruples of the type `Statement` to Ontop triples/quadruples of the
   * type `RDFFact`.
   */
  private RDFFact statementToFact(Statement statement) {
    var sub = statement.getSubject();
    var factSubject = sub.isBNode() ? getTermFactory().getConstantBNode(sub.stringValue())
        : getTermFactory().getConstantIRI(sub.stringValue());
    ...
    var factObject = obj.isBNode()
        ...
        : obj.isIRI()
            ...
            : ((Literal) obj).getLanguage().isPresent()
                ? getTermFactory().getRDFLiteralConstant(((Literal) obj).getLabel(),
                    ((Literal) obj).getLanguage().get())
                : getTermFactory().getRDFLiteralConstant(((Literal) obj).getLabel(),
                    new SimpleRDF().createIRI(((Literal) obj).getDatatype().stringValue()));
    var ctxt = Optional.ofNullable(statement.getContext());
    var factContext = ctxt.map(c -> c.isBNode() ? getTermFactory().getConstantBNode(c.stringValue())
        : getTermFactory().getConstantIRI(c.stringValue()));

    return factContext
        .map(objectConstant -> RDFFactImpl.createQuadFact(factSubject, factPredicate, factObject,
            objectConstant))
