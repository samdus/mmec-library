 ...
 *
 * @brief @~french Implémentation de la classe AvoidJoinAboveUnionPlanner.
 * @brief @~english AvoidJoinAboveUnionPlanner class implementation.
 */
package it.unibz.inf.ontop.iq.planner.impl;

import ca.griis.mmec.controller.ontop.extension.QueryUpdateForMMecOptimizer;
 ...
 */
@Singleton
public class MMecAvoidJoinAboveUnionPlanner implements QueryPlanner {

  private static final Logger LOGGER = LoggerFactory.getLogger(MMecAvoidJoinAboveUnionPlanner.class);
  ...
  private final QueryUpdateForMMecOptimizer queryUpdateForMMecOptimizer;

  @Inject
  protected MMecAvoidJoinAboveUnionPlanner(GeneralStructuralAndSemanticIQOptimizer generalOptimizer,
      AvoidJoinAboveUnionTransformer transformer,
      IntermediateQueryFactory iqFactory, QueryUpdateForMMecOptimizer queryUpdateForMMecOptimizer) {
    this.generalOptimizer = generalOptimizer;
    this.transformer = transformer;
    this.iqFactory = iqFactory;
    this.queryUpdateForMMecOptimizer = queryUpdateForMMecOptimizer;
  }
  ...
  public IQ optimize(IQ query) {
    ...
    return liftedQuery.equals(query)
        ? queryUpdateForMMecOptimizer.optimize(query)
        // Re-applies the structural and semantic optimizations
        : queryUpdateForMMecOptimizer.optimize(generalOptimizer.optimize(liftedQuery));
  }
